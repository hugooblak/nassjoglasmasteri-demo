...
add_filter(
	...
	function ( $exporters ) {
		$exporters['nassjoglasmasteri-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'nassjoglasmasteri-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'nassjoglasmasteri-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'nassjoglasmasteri-leads',
						'group_label' => __( 'Quote requests', 'nassjoglasmasteri-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['nassjoglasmasteri-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'nassjoglasmasteri-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
