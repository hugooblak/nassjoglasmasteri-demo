<?php
/**
 * Nässjö Glasmästeri AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Nässjö Glasmästeri AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through na_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'nassjoglasmasteri', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'nassjoglasmasteri' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'nassjoglasmasteri' ) ) );

		register_block_pattern_category( 'nassjoglasmasteri-sections', array( 'label' => __( 'Nässjö Glasmästeri AB sections', 'nassjoglasmasteri' ) ) );
		register_block_pattern_category( 'nassjoglasmasteri-pages', array( 'label' => __( 'Nässjö Glasmästeri AB page layouts', 'nassjoglasmasteri' ) ) );
	}
);
