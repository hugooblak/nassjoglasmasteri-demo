 ...
 * or photo. They call na_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Nässjö Glasmästeri AB.
 *
 ...
 * @return string
 */
function na_sample_note() {
	return na_lead( 'ar_riktig' ) ? '' : ' <span class="na-sample">(platshållare)</span>';
}
 ...
 * @return string File name for na_lead_img().
 */
function na_lead_bild( $i ) {
	$bilder = (array) na_lead( 'bilder', array() );
 ...
 * @return string
 */
function na_ring_label() {
	$namn = na_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, na_lead( 'telefon' ) ) : 'Ring ' . na_lead( 'telefon' );
}
